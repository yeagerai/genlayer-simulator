...
new_account = None
if (
    "result" in create_account_result
    and "address" in create_account_result["result"]["data"]
):
...
if (
    "result" in fund_account_result
    and "address" in fund_account_result["result"]["data"]
):
...
with open(contract_file_path, "rb") as contract_file:
    deploy_output = deploy_logic(
        new_account, class_name, contract_file, initial_contract_state
    )
...
call_contract_output = contract_logic(
    new_account, last_contract_id, function_to_execute, args
)
