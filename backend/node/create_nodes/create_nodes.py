...
def get_random_provider_using_weights(defaults):
    ...
    if "openai" in provider_weights and (
        "OPENAIKEY" not in os.environ or os.environ["OPENAIKEY"] == default_value
    ):
        provider_weights.pop("openai")
    if "heuristai" in provider_weights and (
        "HEURISTAIAPIKEY" not in os.environ
        or os.environ["HEURISTAIAPIKEY"] == default_value
    ):
...
def random_validator_config(providers: list = []):
    ...
    if (
        ...
        and os.environ["OPENAIKEY"] == "<add_your_open_ai_key_here>"
        and os.environ["HEURISTAIAPIKEY"] == "<add_your_heurist_api_key_here>"
    ):
