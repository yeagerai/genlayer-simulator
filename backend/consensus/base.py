...
class ProposingState(TransactionState):
    ...
    async def handle(self, context):
        ...
        # Create a rollup transaction for the current transaction
...
class RevealingState(TransactionState):
    ...
    async def handle(self, context):
        ...
        for i, validation_result in enumerate(context.validation_results):
            ...
            # Create a dictionary of votes for the current reveal
            single_reveal_votes = {
...
class FinalizingState(TransactionState):
    ...
    async def handle(self, context):
        ...
        # Do not deploy the contract if the transaction was undetermined and the execution failed
        if (context.transaction.status != TransactionStatus.UNDETERMINED) and (
        ...
        # Create a rollup transaction for the finalized transaction
        context.transactions_processor.create_rollup_transaction(
            context.transaction.hash
        )
