...
async function validateTutorialStep({
  ...
  expect(headerText, 'Step title should be visible').to.be.equal(headerElementText)
  const contentText = await content.getText()
  console.log(`Step content: ${contentText}`)

  expect(contentText, 'Step content should be visible').to.be.equal(contentElementText)
  if (isEndStep) {
    ...
    expect(nextButtonText, 'Next button text should be equal to "Next"').to.be.equal(
      nextButtonElementText
    )
